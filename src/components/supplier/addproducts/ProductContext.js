...
const ProductProvider = ({ children }) => {
  ...
  const fetchCategory = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/supplier/category`, {
        method: 'GET',
        headers: {
  ...
  const fetchBrand = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/supplier/brand`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${suppliertoken}`,
