...
export default function Create() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      Swal.fire({ title: "Creating Bad Pincodes...", allowOutsideClick: false, didOpen: Swal.showLoading });
      const formdata = new FormData();
      formdata.append("pincode", formData.pincode);

      const res = await fetch("https://shipowl-kd06.onrender.com/api/bad-pincode", {
        method: "POST",
...
const handleBulkSubmit = async (e) => {
  ...
  try {
    ...
    const res = await fetch("https://shipowl-kd06.onrender.com/api/bad-pincode/import", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="md:w-10/12 p-6 bg-white shadow-md rounded-lg mt-6">
      <h2 className="text-xl font-semibold mb-4">Add Bad Pincodes</h2>
